<form class="ui  form ui justified aligned segment" action="{{ route("request.approval") }}" enctype="multipart/form-data" method="post">
  @if (session('success'))
  ...
  <div class="field {{ $errors->has('name') ? 'error' : '' }}">
    ...
    <input value="{{ old('name') }}" type="text" name="name" id="name" placeholder="Application Name" >
  </div>
        ...
        @for ($i=0; $i < 7; $i++)
            <div class="col-md-6">
              <div class="field {{ $errors->has('workingDrawings') ? 'error' : '' }}">
                ...
                <input value="{{ old('working_drawings') }}" type="file" name="{{$filed[$i]}}" class="dropify working_drawings" id="p-image" >
              </div>
            </div>
        @endfor
       <hr>
  ...
  <div class="ui submit button teal">Submit</div>
</form>
